...
```
+----------------------------------------------------------+
|                    CLIENT MACHINES (Linux)               |
|  - Linux systems (workstations/servers)                  |
|  - Use SSSD to authenticate with FreeIPA for user login  |
|  - Enforce HBAC policies for host-based access control   |
|  - Access services via Kerberos (SSO)                    |
+----------------------------------------------------------+
                          |
                          |
                +------------------+
                |    FreeIPA       |  <--- Central Identity & Domain Services
                |  (LDAP, Kerberos,|         - User & group management
                |   HBAC, CA)      |         - Centralized authentication
                |                  |         - MFA & Access Control
                |                  |
                +------------------+
                          |
                ...
                | (OIDC, SAML, MFA)|         - Authentication for web apps
...
|  - Web-based services (e.g., intranet apps, internal APIs) |
|  - Authenticate users via Keycloak (OIDC/SAML)             |
|  - User information pulled from FreeIPA via Keycloak LDAP  |
+------------------------------------------------------------+
                          |
                          |
                +------------------+
                |    LDAP Clients  |  <--- FreeIPA Backend (LDAP)
                | (SSSD, Kerberos) |         - Centralized user authentication
                ...
                |    Monitoring    |         - Collect logs from FreeIPA & Keycloak
                |  (Graylog, ELK)  |         - Monitor access events, failed logins, MFA attempts
                +------------------+
```
